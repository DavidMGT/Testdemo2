...
public class AddNoticeOneActivity extends Activity implements OnClickListener, OnLoadingDialogResultListener {
    private static final int UPLOAD_RECORD_SUCESS = 0x001;//上传录音文件
    ;
    ...
    public void NextStep() {
        ...
        AjaxParams params = servecHttp.noticeOne(key, userId, title, content, "/" + pathName, feedback_one,
                feedback_two);
        finalHttp.post(IBaes.NOTICE_ONE, params, new AjaxCallBack<String>() {
            @Override
            public void onSuccess(String t) {
                super.onSuccess(t);
                LogUtils.debug("提交通知-------"+t);
    ...
    public void initRecord() {
        ...
        file.mkdirs();// 创建文件夹
        fileName = path + "/info_" + System.currentTimeMillis() + ".aac";
    ...
    public void uploadRecord() {
        if (!"".equals(path)) {
            ...
            finalHttp.post(IBaes.NOTICE_UPLOAD, params, new AjaxCallBack<String>() {
                @Override
                public void onSuccess(String t) {
                    // TODO Auto-generated method stub
                    super.onSuccess(t);
                    LogUtils.debug("UPLOAD_RECORD---" + t);
                    // Log.i("test", t.toString());
                    Map<String, Object> map = jsonDate.jsonNoticeUpload(t.toString());
                    Message msg = new Message();
                    msg.obj = map;
                    msg.what = UPLOAD_RECORD_SUCESS;
                    handler.sendMessage(msg);
                }
            });
        }
            ...
            switch (msg.what) {
                case 0:
                    ...
                    if (status == 1) {
                        ...
                        Intent intent = new Intent(AddNoticeOneActivity.this, SelectClassActivity.class);
